...
class HomeController extends Controller
{
        ...
        if (Auth::user()->access_level == 2) {
            $cardNumber = \DB::select("SELECT * FROM (
                (
                    ...
                    LEFT JOIN transaction_type b
                    ON a.transaction_type_id = b.id
                    LEFT JOIN email_tickets_meta d
                    ON a.email_tickets_meta = d.ticket_id
                    WHERE b.transaction_type = 'Prod'
                    AND d.meta_state != 'Unworkable'
                    AND a.user_id = ?
                    AND start_datetime BETWEEN ? and ?) AS T2,

                    (SELECT count(a.transaction_type_id) as Aux 
                    FROM transaction a
                    LEFT JOIN email_tickets_meta b
                    ON a.email_tickets_meta = b.ticket_id
                    WHERE b.meta_state = 'Unworkable'
                    AND a.user_id = ?
                    AND start_datetime BETWEEN ? and ?) AS T3))", 
                    [
                        Auth::user()->id,
                        $cur_start,
                        $cur_end,
    ...
    public function myteamTicket(){
        $myTeamTickets = \DB::select(
            " SELECT a.ticket_code, b.transaction_type_id, b.total_tat, u.name, u.call_sign, a.queue, e.specification, a.dt_locked, a.start_process_time
